...
const PAGE_TEMPLATE_NATIONAL = (careerKeyword: string, careerTitle: string) => `import { prisma } from "@/lib/prisma";
...
export const metadata = {
    title: "${careerTitle} Salary in United States (2025 Guide)",
    description: "How much does a ${careerTitle} make in the United States? View average salary, hourly pay, and wage distribution for 2025.",
};
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                            {relatedStates.map((state: any) => (
                                <Button key={state.id} variant="outline" size="sm" asChild>
                                    <Link href={\`/${careerKeyword}-salary/\${state.slug}\`}>
                                        {state.stateName}
                                    </Link>
                                </Button>
                            ))}
...
async function main() {
    console.log('🔍 Fetching salary data from database...');

    // Get all unique career keywords with national data
    ...
    console.log(`✅ Found ${careers.length} careers with national data`);
    console.log('Careers:', careers.map(c => c.careerKeyword).join(', '));
    ...
    // Create national pages for each career
    for (const career of careers) {
        const careerKeyword = career.careerKeyword;
        const careerTitle = formatCareerTitle(careerKeyword);

        const folderPath = path.join(process.cwd(), 'src', 'app', '(pSEO)', `${careerKeyword}-salary`);
        ...
        fs.writeFileSync(pagePath, PAGE_TEMPLATE_NATIONAL(careerKeyword, careerTitle));
        console.log(`✅ Created: ${careerKeyword}-salary/page.tsx`);
        pagesCreated++;
    }

    console.log(`\n🎉 Created ${pagesCreated} salary pages!`);
    console.log('\nNext steps:');
    console.log('1. Run: git add -A && git commit -m "feat: generate salary pages for all careers"');

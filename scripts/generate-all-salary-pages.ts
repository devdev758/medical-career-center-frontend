...
const OUTPUT_DIR = path.join(process.cwd(), 'src', 'app', '(pSEO)');
...
// Helper function to format career title
function formatCareerTitle(title: string): string {
    // Remove "All Other" and clean up
    return title
        .replace(', All Other Specialists', '')
        .replace(', All Other', '')
        .trim();
}

// Page template for state pages
const STATE_PAGE_TEMPLATE = (data: any) => `import { prisma } from "@/lib/prisma";
...
export const metadata = {
    title: "${data.careerTitle} Salary in ${data.stateName} (2025 Guide)",
    description: "How much does a ${data.careerTitle} make in ${data.stateName}? View average salary, hourly pay, and wage distribution for 2025.",
};

export default async function Page() {
    const salaryData = await prisma.salaryData.findFirst({
        where: {
            careerKeyword: "${data.careerSlug}",
            location: { slug: "${data.stateSlug}" },
            year: 2024
        },
    ...
    const careerTitle = "${data.careerTitle}";
    const locationName = "${data.stateName}";
    ...
    const careerDescription = getCareerDescription("${data.careerSlug}");
            ...
            <Card className="bg-primary text-primary-foreground my-12">
                <CardContent className="p-8 text-center space-y-4">
                    ...
                    <Button size="lg" variant="secondary" asChild className="mt-4">
                        <Link href="/${data.careerSlug}-jobs/${data.stateSlug}">
...
async function generatePages() {
    ...
    // Filter for state-level data only (AREA_TYPE = 2 for states)
    const stateData = data.filter((row: any) =>
        row.AREA_TYPE === '2' &&
        row.AREA_TITLE !== 'U.S.' &&
        row.O_GROUP === 'detailed' // Only detailed occupations, not broad categories
    );

    console.log(`✅ Filtered to ${stateData.length} state-level profession records\n`);
    ...
    for (const row of stateData) {
        try {
            const careerTitle = formatCareerTitle(row.OCC_TITLE);
            const careerSlug = createSlug(careerTitle);
            const stateName = row.AREA_TITLE;
            const stateSlug = createSlug(stateName);

            // Create folder structure
            const folderPath = path.join(OUTPUT_DIR, `${careerSlug}-salary`, stateSlug);
            ...
            // Generate page
            const pageContent = STATE_PAGE_TEMPLATE({
                careerTitle,
                careerSlug,
                stateName,
                stateSlug
            ...
            if (pagesCreated % 100 === 0) {
                console.log(`✅ Created ${pagesCreated} pages...`);
            }
        } catch (error) {
            errors++;
            console.error(`❌ Error creating page for ${row.OCC_TITLE} in ${row.AREA_TITLE}:`, error);
        }
    }
    ...
    console.log(`\nNext steps:`);
    console.log(`1. Review a few generated pages`);
    console.log(`2. Run: git add src/app/(pSEO)`);
    console.log(`3. Run: git commit -m "feat: generate salary pages for all professions and states"`);

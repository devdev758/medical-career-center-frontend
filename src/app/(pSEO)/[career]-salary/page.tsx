...
export async function generateMetadata({ params }: PageProps) {
    const careerTitle = formatCareerTitle(params.career);
...
export default async function Page({ params }: PageProps) {
    ...
    const careerDescription = getCareerDescription(params.career);
                        ...
                        <div className="flex flex-wrap gap-2">
                            {relatedStates.map((state) => (
                                <Button key={state.id} variant="outline" size="sm" asChild>
                                    <Link href={`/${params.career}-salary/${state.slug}`}>
                                        {state.stateName}
                                    </Link>
                                </Button>
                            ))}

...
export function RelatedSalaries({ currentProfession, state, city, availableSlugs = [] }: RelatedSalariesProps) {
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {displayCareers.map((career) => {
                    const isCityAvailable = availableSlugs.includes(career.slug);
                    const href = isCityAvailable && city && state
                        ? `/${career.slug}/salary/${state.toLowerCase()}/${city.toLowerCase()}`
                        : `/${career.slug}/salary${state ? '/' + state.toLowerCase() : ''}`;
